...
{
    Debug.Log($"Event received: {e}, Current State: {onlineStateController.currentState}");

    if (!onlineStateController.validate(e))
    {
        Debug.LogWarning($"Unexpected event: {e} in state: {onlineStateController.currentState}");
    }

    if (isCancelled)
    {
        Debug.Log("Search process cancelled.");
        return;
    }
